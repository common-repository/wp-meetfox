	...
	protected function build_settings_field( $page_key, $section_key, $field_key ) {
		...
		switch ( $field['type'] ) {
			...
			case 'switch':
				...
				printf(
					'<div class="switch"><label><input %s %s id="%s" name="%s" title="%s" type="checkbox" value="%s"><span class="slider round"></span>&nbsp; %s</label></div>',
					$checked,																			// checked
					!empty( $field['class'] ) ? "class='{$field['class']}'" : '',						// class
					$field['id'],																		// id
					"{$page_key}[{$field['id']}]",														// name
			...
			case 'range':
				printf(
					'<div style="display: flex; align-items: center"><span>%s</span><input min=1 max=60 onchange="this.previousElementSibling.innerHTML = this.value" %s id="%s" name="%s" %s title="%s" type="%s" value="%s" %s>%s</div>',
					$field['value'],
					!empty( $field['class'] ) ? "class='{$field['class']}'" : '',						// class
					$field['id'],																		// id
					"{$page_key}[{$field['id']}]",														// name
